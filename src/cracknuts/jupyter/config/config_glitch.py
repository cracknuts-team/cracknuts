...
from cracknuts.cracker.cracker_g1 import (
    CrackerG1,
    wave_4m,
    wave_8m,
    wave_10m,
...
class ConfigG1Glitch(MsgHandlerPanelWidget):
    ...
    # These two attributes are used to synchronize waveform related settings
    # made in Python code with the Jupyter interface.
    ...
    glitch_clock_normal_freq_items: list[dict[str, typing.Any]] = traitlets.List(
        [
            {"name": f"{name}-[{', '.join(map(str, wave))}]", "tooltip": f"[{', '.join(map(str, wave))}]", "wave": wave}
            for name, wave in _glitch_clock_embed_wave.items()
        ]
    ).tag(sync=True)
    ...
    def _glitch_clock_freq_add(self, new_wave):
        ...
        glitch_clock_freq_items.append(
            {
                "name": f"C-{len(glitch_clock_freq_items) - len(self._glitch_clock_embed_wave)}"
                f"-[{", ".join(map(str, new_wave))}]",
