...
class TraceMonitorPanelWidget(MsgHandlerPanelWidget):
    ...
    y_range: dict[int, tuple[int, int, int]] = traitlets.Dict({1: (None, None, None), 2: (None, None, None)}).tag(
        sync=True
    )
    ...
    def update(self, series_data: dict[int, np.ndarray]) -> None:
        mn1, mx1, interval1 = None, None, None
        mn2, mx2, interval2 = None, None, None

        if self.custom_range_model:
            mn1, mx1 = self.custom_y_range["1"]
            mn2, mx2 = self.custom_y_range["2"]
        else:
