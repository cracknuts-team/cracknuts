...
def __(mo):
    mo.md(
        ...
        这里给出一个对软件实现的AES算法进行CPA分析的例子。例子中的AES算法运行于STM32F1，使用的是mbedTLS的查小表实现。对于采集到的能量迹，我们使用zarr格式存储（参见<https://zarr.readthedocs.io/en/stable/tutorial.html>）。
        """
    )
    return
...
@app.cell
def __(get_selected_file_path, mo):
    get_dataset_name, set_dataset_name = mo.state(None)
    def start_analysis(_):
        set_dataset_name(get_selected_file_path())
    button_start_analysis = mo.ui.button(label='分析', on_click=start_analysis)
    return (
        button_start_analysis,
        get_dataset_name,
        set_dataset_name,
        start_analysis,
    )
...
def __():
    ...
    # dataset = zarr.open(get_dataset_name(), "r")
    return
...
@app.cell
def __(get_selected_origin_data, mo):
    mo.stop(get_selected_origin_data() is None)
    get_selected_origin_data().info
    return
...
@app.cell
def __(button_start_analysis, mo):
    mo.md(f"""打开能量迹文件对其进行分析，点击{button_start_analysis}开始。""")
    return


@app.cell
def __(Container, ContainerOptions, SboxWeight, cpa, get_dataset_name, th):
    handler2 = th(fileName=get_dataset_name())
    model = SboxWeight()
    engine2 = cpa(model)
...
def __(get_selected_origin_data, mo):
    ...
    data = get_selected_origin_data()['0/0/plaintext'][:,0]
    #将曲线取出
    traces = get_selected_origin_data()['0/0/traces']
    return data, traces
...
def __():
    ...
    return Blosc, alt, cracknuts, mo, np, os, pd, plt, stdout, trace, zarr
...
def __(
    ...
    def get_select_file_trace_attr(file_path):
        ...
        if file_path.endswith('.npy'):
            data_type, traces_source, trace_count, sample_count, data, _ = trace.load_traces(file_path)
            set_selected_origin_data(_)
            ...
            if file_path:
                set_selected_file_path(file_path)
                data_type, traces_source, trace_count, sample_count, data, origin_data = trace.load_traces(file_path)
                set_selected_origin_data(origin_data)
                set_selected_trace_count(trace_count)
                set_selected_sample_count(sample_count)
                set_selected_data(data)
                set_trace_count(trace_count)
...
def __(mo, np, pd):
    ...
    get_selected_file_path, set_selected_file_path = mo.state(None)
    get_selected_origin_data, set_selected_origin_data = mo.state(None)
